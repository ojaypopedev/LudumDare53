...
namespace HotDogCannon.Utils
{
    public class PositionAnims : MonoBehaviour
    {
        ...
        IEnumerator DoAnim()
        {
            ...
            while (elapsedTime < time)
            {
                ...
                model.position = Vector3.Lerp(startPos, targetPos == null ? targetPosV3 : targetPos.position, dt);
            ...
            model.position = targetPos == null? targetPosV3 : targetPos.position;
            callback?.Invoke();
            Destroy(gameObject);
        }
    ...
    public static class PosAnims
    {
        ...
        public static void AnimatPos(Transform model, Vector3 startPos, Vector3 targetPos, float time, System.Action callback = null)
        {
            var animObj = new GameObject("posAnim");
            ...
            posAnimScript.model = model;
            posAnimScript.startPos = startPos;
            posAnimScript.targetPosV3 = targetPos;
            posAnimScript.time = time;
            posAnimScript.callback = callback;
            posAnimScript.StartAnim();
        }
